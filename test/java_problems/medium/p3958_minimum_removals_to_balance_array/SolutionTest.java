...
class SolutionTest {
    ...
    void constructTransformedArray(int[] nums, int k, int expected) {
        Solution solution = new Solution();
        Assertions.assertEquals(expected, solution.minRemoval(nums, k));
    }
    ...
    static Stream<Arguments> getLargeArrayProvider(String first) throws IOException {
        ...
        int[] array = new int[parts.length - 2];
        for (int i = 0; i < parts.length - 2; i++) {
            array[i] = Integer.parseInt(parts[i].trim());
        }

        int k = Integer.parseInt(parts[parts.length - 2].trim());
        int expected = Integer.parseInt(parts[parts.length - 1].trim());

        return Stream.of(Arguments.of(array, k, expected));
    }

    static Stream<Arguments> largeArrayProvider980() throws IOException {
        return getLargeArrayProvider("test/java_problems/medium/p3958_minimum_removals_to_balance_array/data980.csv");
    }

    @ParameterizedTest
    @MethodSource("largeArrayProvider980")
    void constructTransformedArray980(int[] nums, int k, int expected) {
        Solution solution = new Solution();
        Assertions.assertEquals(expected, solution.minRemoval(nums, k));
    }

    static Stream<Arguments> largeArrayProvider982() throws IOException {
        return getLargeArrayProvider("test/java_problems/medium/p3958_minimum_removals_to_balance_array/data982.csv");
    }

    @ParameterizedTest
    @MethodSource("largeArrayProvider982")
    void constructTransformedArray982(int[] nums, int k, int expected) {
        Solution solution = new Solution();
        Assertions.assertEquals(expected, solution.minRemoval(nums, k));
    }

    static Stream<Arguments> largeArrayProvider983() throws IOException {
        return getLargeArrayProvider("test/java_problems/medium/p3958_minimum_removals_to_balance_array/data983.csv");
    }

    @ParameterizedTest
    @MethodSource("largeArrayProvider983")
    void constructTransformedArray983(int[] nums, int k, int expected) {
        Solution solution = new Solution();
        Assertions.assertEquals(expected, solution.minRemoval(nums, k));
    }
